...
from backend.app.services.lawllm_service import LawLLMService

# 设置日志
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

async def test_lawllm():
    """测试 LawLLM-7B 模型"""
    logger.info("🧪 开始测试 LawLLM-7B 模型")
    logger.info("=" * 50)
    
    try:
        # 1. 初始化服务
        logger.info("1. 初始化 LawLLM-7B 服务...")
        service = LawLLMService()
        service.initialize()
        ...
        logger.info(f"采样参数: {model_info['sampling_params']}")
